...
#define MDR32FxQI_SPL_VERSION                                                  \
  ((_MDR32FxQI_SPL_VERSION_MAJOR << 16) |                                      \
   (_MDR32FxQI_SPL_VERSION_MINOR << 8) | (_MDR32FxQI_SPL_VERSION_PATCH))
...
// </h>
    ...
    (defined(USE_K1986VE1xI) && !defined(USE_MDR32F1QI))
// <h> Factory data management for MCU K1986VE92xI (K1986VE92FI, K1986VE92F1I),
// K1986VE94GI and K1986VE1xI (K1986VE1FI, K1986VE1GI) <o> Factory data
// initialization in SystemInit() (called before entering main())
//   <0=> 0: REG_0E (Trim[2:0] bits) and REG_0F (HSI TRIM[5:0] and LSI TRIM[4:0]
//   bits) are NOT initialized from factory data. <1=> 1: REG_0E (Trim[2:0]
//   bits) and REG_0F (HSI TRIM[5:0] and LSI TRIM[4:0] bits) are initialized
//   from factory data.
// <i> Default: 1 (Use initialization).
// <i> ATTENTION: Factory data stored in EEPROM must ALWAYS be used to ensure
// the correct operation of MCU. Setting this parameter to 0 is allowed only if
// the use of factory data is implemented elsewhere in the software.
...
#error                                                                         \
    "DMA_AlternateData  should be 0 (DMA_ALternateDataDisabled) or 1 (DMA_ALternateDataEnabled)"
#endif

#if (DMA_AlternateData == 0)
// <o.0..5> Number of DMA channels main structure to use.
// <i> Specifies size of structure table for channels in range [0;
// DMA_Channels_Number-1] where 0 is channel 0. <i> Could be changed to reduce
// RAM usage. <i> If DMA_AlternateData = 1, 32 channels for main structure are
// always used. <i> This parameter is in range [1; 32] <i> Default: 32
...
// <i> Specifies size of structure table for channels in range [0; 32 +
// DMA_Channels_Number-1] where 0 is channel 0. <i> Could be changed to reduce
// RAM usage. <i> If DMA_AlternateData = 1, 32 channels for main structure are
// always used. <i> This parameter is in range [1; 32] <i> Default: 32
#define DMA_Channels_Number 32
...
/* Uncomment one of the lines below to select the Device Class. Leave all
 * commented if the desired device class is not currently supported by USB
 * library */
 ...
 * Leave this line commented if you are willing to implement the handler
 * yourself. */
...
// <c2> MDR32F1QI, K1986VE1xI Series Errata Notice, ETH PHY data inversion error
// in 10Base-T mode <i>  Workaround for "10Base-T HD/FD transmitted data
// inverted after EthernetPHY reset" error <i> Default: not commented/defined
...
//   <2=> 2: check enabled, source file ID, line number and checking expression
 ...
 *         and expression text (if USE_ASSERT_INFO == 2) of the call that
 * failed. That function should not return. If expr is true, nothing is done.
 * @retval None
 */
...
#define assert_param(expr)                                                     \
  ((expr)                                                                      \
       ? (void)0U                                                              \
       : assert_failed((uint8_t *)__FILE__, __LINE__, (const uint8_t *)#expr))
